...
Jh.Util = {//工具类
	...
	toBody:function(o){
		//$("body").append(o);//往Body添加对象
		$(".fallr_windows").append(o);
	}
};
Jh.fn = function(me){//功能区
	return me = {
		...
		_create:function(){//创建自己
			...
			$(".fallr_windowsfoot").append(_box);  //再另一个div中加入
		},
		...
		_createActionButton:function(){//创建功能按钮
			var _add = $("<a class='button b' href='javascript:void(0)' >\u6DFB\u52A0\u6A21\u5757</a>");
			var _save= $("<a class='button b' href='javascript:void(0)' >\u4FDD\u5B58\u914D\u7F6E</a>");
		...
		_createLi:function(key,name){//创建li
			return $("<li/>").append("<a href='javascript:void(0)' rel='"+key+"'>"+name+"</a>")
	...
	return me={	
		...
		_createPortalOne:function(key,name){//创建单个portal item
			...
			var itemHeader  =  me._createItemHeader(name),//header
			    itemContent =  me._createItemContent(key),//content
		...
		_createItemContent:function(key){//创建content
			var _content = $(_template.itemContent);
			$("<ul style='width:250px; height:150px;' id='ul"+key+"' ><li id='li"+key+"' >闻到食府网功能正在开发中</li></ul>").appendTo(_content);
			return _content;
		},
