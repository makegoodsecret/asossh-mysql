...
//名片夹代码
	...
	function  ajaxsubmitimage(formName,hotelimageidvalue,hotelimageid,hotelimgshow,hotelimgsubmitid){
		...
		function resultName(){
			...
			if(hotelName==""){
				 messagerShow("<font color='RED' size='2'>饭店名称不能为空</font>");
		...
		function resultAds(){
			...
			if(hotelAds==""){
			 	messagerShow("<font color='RED' size='2'>饭店描述不能为空</font>");
		...
		function resultuserinputaddress(){
			//注入地址
			var province="请选择" ,city="请选择",county="请选择";
			  province = $("#province").val().trim();
		 		if(province=="请选择"){
				  messagerShow("<font color='RED' size='2'>收件人地址不完整</font>");
				 return false;
				}else{
			...
			var otherAddress = $("#userinputaddress").val().trim();
		 	if (otherAddress.length<=0) {
		 		 messagerShow("<font color='RED' size='2'>收件人地址(不包含省市区)不完整</font>");
				 $("#msghotelAddress").html("<font color='RED' size='2'>收件人地址(不包含省市区)不完整</font>");
				 return false;
			}else if (isNaN(otherAddress)&&otherAddress.indexOf(".")<-1) {
				 messagerShow("<font color='RED' size='2'>收件人地址(不包含省市区)不能为纯数字</font>");
		...
		function resultPhone(){
			...
			if(hotelPhone==""){
				 messagerShow("<font color='RED' size='2'>饭店联系手机不能为空</font>");
		...
		if(hotelShopPMStart.length==0||hotelShopPMEnd.length==0){
			 messagerShow("<font color='RED' size='2'>饭店营业时间晚餐不能为空</font>");
		...
		function  resultHotelImge(){
			...
			if (idhotelLogo.length==0) {
				 messagerShow("<font color='RED' size='2'>请上传饭店logo</font>");
				$("#msghotelImage").html("<font color='RED' size='2'>请上传饭店logo</font>");
				return false;		
			}else if (idhotelCertificate.length==0) {
				 messagerShow("<font color='RED' size='2'>请上传饭店许可证图片</font>");
	...
	function resultGpspoint(){
			if($("#info_lng").val()==""||$("#info_lng").val()==""){
				 messagerShow("<font color='RED' size='2'>饭店地理位置不能为空</font>");
	...
	$("#areacode").focus(function(){
	 ...
	 function  hoteluseremailstatu(){
	 	...
	 	if (hotelusermailsatu!=2) {
	 		 messagerShow("<font color='RED' size='2'>此人员权限不足，无法管理饭店,请重新分配人员</font>");
			return  false;
		}else{
		   return  true;
		}
	...
	$("#saveHotel").click(function(){
		...
		if (userpower==1) {  //管理员
			var username=dojo.widget.byId("auto").getText();
		 	$("#hoteluserPojouserId").val(username); //注入username
		 	$.getJSON(		//注入人员的权限
			"<%=basePath%>asossh/userinfo!getjsonuserinfo.action",
			{"userPojo.username":username},function(data){
				$("#hotelusermailsatu").val( data.useremailsatu);
			});
		} 
		if (resultName()&&resultAds()&&resultShopHours()&&resultuserinputaddress()
			&&resultPhone()&&resultGpspoint()&&resultHotelImge()&&hoteluseremailstatu()&&payname()){
		 	formhotelsave.action="<%=basePath%>asossh/hotelmanage!savehotel.action";
		 	formhotelsave.submit();
		}
	});
	$("#userinfobutton").click( function(){
		var  username=dojo.widget.byId("auto").getText();
		if (username=="") {
			 messagerShow("<font color='RED' size='2'>请先选择载参看</font>");
		}else{
			$.getJSON(
			"<%=basePath%>asossh/userinfo!getjsonuserinfo.action",
  ...
  <body  background="<%=basePath %>image/bg.jpg" >
	...
	<table background="<%=basePath %>image/bg.jpg" align="center" width="90%">
		...
		<!-- 添加饭店  -->
			<td width="50%">
				<form id="formhotelsave" name="formhotelsave" action="<%=basePath%>asossh/hotelmanage!savehotel.action" method="post" >
				<table width="100%" border="0" style="border:1px solid #abcdef;">
					<tr>
						...
						<td width="70%">
							<input type="text" id="hotelName" name="hotelPojo.hotelName" style="font-size: 1pc;"> 
						</td>
					</tr>
					...
					<tr>
						...
						<td width="70%">
						<textarea rows="2" cols="30"  id="hotelAds" name="hotelPojo.hotelAds"></textarea>
						</td>
					</tr>
					<tr>
					<td></td>
					...
					<tr>
						...
						<td width="70%">
						中餐：<input type="text" id="hotelShopAMStart" name="hotelPojo.hotelShopAmStart" size="8px;" style="font-size: 1pc;">
						至<input type="text" id="hotelShopAMEnd" name="hotelPojo.hotelShopAmEnd" size="8px;" style="font-size: 1pc;"><br>
						<br>
					  	晚餐：<input type="text" id="hotelShopPMStart" name="hotelPojo.hotelShopPmStart" size="8px;" style="font-size: 1pc;">
					  	至<input type="text" id="hotelShopPMEnd" name="hotelPojo.hotelShopPmEnd" size="8px;" style="font-size: 1pc;">
					    <script>
					...
					<tr>
						...
						<textarea rows="2" cols="20" id="userinputaddress" name="hotelPojo.hotelOtherAddress"></textarea>
						 <input type="hidden" id="hotelProvince" name="hotelPojo.hotelProvince">
						 <input type="hidden" id="hotelCity" name="hotelPojo.hotelCity">
						 <input type="hidden" id="hotelCounty" name="hotelPojo.hotelCounty">
						 </td>
					</tr>
					...
					<tr>
						...
						<td width="70%">
							<input type="text" id="hotelPhone" name="hotelPojo.hotelCellphone" style="margin-top: 5px; margin-bottom: 10px;"> <br>
							<input type="text" id="areacode" name="hotelPojo.hotelPhonesection" size="3" style="font-size: 1pc;" >
							<input type="text" id="telephone" name="hotelPojo.hotelPhonecode"size="10" style="font-size: 1pc;" >
							<input type="text" id="mobilephone" name="hotelPojo.hotelOtherphone" size="10"style="font-size: 1pc;"><br>
						</td>
					</tr>
					...
					<tr>
						...
						<td>
							<input type="text"  id="paynameid" value="" name="userPojo.payname">
							<br>注意:<br>
							<em style="font-family: 楷体;font-size: 15px;">
							1.如果查看了该饭店管理员，将会自动填充.<br>
							2.如果填写了错误的支付宝号，导致顾客的投诉，将对你的饭店做停业处理.
							</em>
							<input type="hidden"   id="hoteluserPojouserId" name="userPojo.username">
							<input type="hidden"   id="hotelusermailsatu" value="0" >
					...
					<tr>
						<td colspan="2" align="center">
							<button  type="button" style="padding: 10px 10px ; margin-right: 10px; " 
							 id="saveHotel">创建饭店</button>
							...
							id="index">返回</button>
			<!-- 隐藏提交经纬度 -->
						<input type="hidden"    name="hotelPojo.hotelLng"  id="info_lngs"    >
						<input type="hidden"    name="hotelPojo.hotelLat"  id="info_lats"    >
			<!-- 隐藏提交图片 -->
						<input type="hidden"    name="hotelPojo.hotelLogo"  id="idhotelLogo"    >
						<input type="hidden"    name="hotelPojo.hotelCertificate"  id="idhotelCertificate"    >
						</td>
					</tr>
				</table>
				</form>
		...
		<!-- 百度地图位置定位 -->
			<td width="50%">
				<div style="width: 100% ; height: 160px;" >
					<div style="float:left; border:1px solid #abcdef; display: inline;width: 30%  ; height: 160px;">
						<form id="formhotelLogo" method="post" enctype="multipart/form-data">
							...
							<input type="hidden" id="hotelLogonameid" name="imagesname" >
							</span>
						</form>
						<hr>
						...
						<span>
							...
							<input type="hidden" id="hotelCertificatenameid" name="imagesname" >
						</span>
						</form>
					</div>
					<div style="float:left; border:1px solid #abcdef; display: inline; width: 34% ; height: 160px;">
						<img alt="选择上传饭店的logo" src=""  width="100%" height="160px;" id="hotellogoimg">
					</div>
					<div style="float:left; border:1px solid #abcdef; display: inline; width: 34% ; height: 160px;">
						<img alt="选择上传饭店的许可证" src=""  width="100%" height="160px;" id="hotelCertificateimg">
					</div>
				</div>
				...
				<input type="text"      id="info_lng" style="display: none" disabled="disabled"  >
				<input type="text"       id="info_lat" style="display: none" disabled="disabled"  >
				</div>
				<div id="l-map"></div>
			</td>
		</tr>		
...
<script type="text/javascript">
   ...
   var point  = new BMap.Point(116.331398,39.897445);   	//默认为浙江海洋学院
   ...
   function myCity(result){
   	...
   	map.setCenter(cityName);        						 //将显示的位置定位到当前访问者的城市
   	//alert(cityName);
   }
   ...
   var txtMenuItem = [{text:'放大',callback:function(){ map.zoomIn(); }},
	  ...
	  {text:'在此添加标注',callback:function(p){inmarker(p);map.removeEventListener("click",showPoin); //移除标记,及点击别处不会生成新的
	}}];
